...
class SlotRequestsController extends Controller
{
    ...
    public function teacherMySlot($id){
        ...
        $conversations= DB::table('conversations')->join('conver_users','conver_users.conver_id','=','conversations.id')->where('conver_users.user_id','=',Auth::user()->id)->where('conversations.slot_id','=',$id)->select('conversations.id')->get();
        error_log( 'cons are'.sizeof($conversations));
        return  view('auth.mySlot')->with('teacher',True)->with('students',$students)->with('conver',$conversations)->with('slot_id',$id);
    }
    ...
    public function getStudentConversations($id){
        ...
        $conversations= DB::table('conversations')->join('conver_users','conver_users.conver_id','=','conversations.id')->where('conver_users.user_id','=',Auth::user()->id)->where('conversations.status','=','active')->where('slot_id','=',$id)->select('conversations.id')->get();
        error_log( 'cons are'.sizeof($conversations));
